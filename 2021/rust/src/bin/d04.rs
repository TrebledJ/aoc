...
fn at(i: usize, j: usize) -> usize { return j * 5 + i; }
fn index2(i: usize) -> (usize, usize) { return (i % 5, i / 5); }
...
fn grid_score(grid: &Grid, mark: &Vec<bool>, last: &u32) -> u32 {
    return grid.iter().zip(mark).map(|(x, m)| x * (!m as u32)).sum::<u32>() * last;
}
...
fn main() {
    ...
    let grids/* : Vec<Grid> */ = sections
        .map(|g| g.split(char::is_whitespace).filter(|x| !x.is_empty()).map(|x| x.parse::<u32>().unwrap()).collect::<Vec<_>>())
        .collect::<Vec<_>>();
