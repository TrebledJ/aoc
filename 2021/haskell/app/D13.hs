...
parser = (,) <$> some (coor <* newline) <* newline <*> folds `sepBy1` newline <* eof
  where coor = (,) <$> number <* char ',' <*> number
        folds = (,) <$ string "fold along " <*> (axis <$> letterChar) <* char '=' <*> number
        axis 'x' = X
        axis _ = Y
...
part2 (cs, fs) = trace grid 0
  where dots = foldl foldPaper (S.fromList cs) fs
        grid = unlines $ makeGrid dots
  
foldPaper :: S.HashSet Coor -> Fold -> S.HashSet Coor
foldPaper cs (ax, v) = S.map mapCoor cs 
  where mapCoor (x, y) | ax == X = if x >= v then (x - 2 * abs (x - v), y) else (x, y)
                       | otherwise = if y >= v then (x, y - 2 * abs (y - v)) else (x, y)

makeGrid :: S.HashSet Coor -> [String]
makeGrid cs = [[if (i, j) `elem` cs then '#' else '.' | i <- [w1..w2]] | j <- [h1..h2]]
  where w1 = minimum $ (S.map fst) cs
